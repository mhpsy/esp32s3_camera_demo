...
static esp_err_t _update_config_from_descriptor(const usb_config_desc_t *cfg_desc)
{
    ...
    uint8_t best_frame_idx = 0;
    uint32_t best_frame_area = 0;
            ...
            if (context_class == USB_CLASS_VIDEO && context_subclass == VIDEO_SUBCLASS_CONTROL) {
                ...
                if (context_intf_alt == 0) {
                    ...
                    case VIDEO_CS_ITF_VS_FRAME_MJPEG: {
                        ...
                        bool within_req = true;
                        if (usb_dev->uvc_cfg.frame_width != FRAME_RESOLUTION_ANY && _frame_width > usb_dev->uvc_cfg.frame_width) {
                            within_req = false;
                        }
                        if (usb_dev->uvc_cfg.frame_height != FRAME_RESOLUTION_ANY && _frame_heigh > usb_dev->uvc_cfg.frame_height) {
                            within_req = false;
                        }
                        if (within_req) {
                            uint32_t area = (uint32_t)_frame_width * (uint32_t)_frame_heigh;
                            if (area > best_frame_area) {
                                best_frame_area = area;
                                best_frame_idx = _frame_idx;
                            }
                        }
    ...
    if (usb_dev->enabled[STREAM_UVC]) {
        ...
        if (user_frame_found) {
            ...
            UVC_EXIT_CRITICAL();
        } else if (best_frame_idx != 0) {
            UVC_ENTER_CRITICAL();
            uvc_dev->frame_index = best_frame_idx;
            uvc_dev->frame_height = uvc_dev->frame_size[best_frame_idx - 1].height;
            uvc_dev->frame_width = uvc_dev->frame_size[best_frame_idx - 1].width;
            UVC_EXIT_CRITICAL();
            ESP_LOGW(TAG, "Frame not found, fallback to best <= requested: %u*%u (index=%u)",
                     uvc_dev->frame_width, uvc_dev->frame_height, best_frame_idx);
        } else {
    ...
    if (!flag_zlp) {
        ...
        if (payload_len >= payload[0]
            ...
            if (header_info & 0x40) {
                ...
                uint8_t b4 = (payload_len > 4) ? payload[4] : 0;
                uint8_t b5 = (payload_len > 5) ? payload[5] : 0;
                uint8_t b6 = (payload_len > 6) ? payload[6] : 0;
                uint8_t b7 = (payload_len > 7) ? payload[7] : 0;
                uint8_t t0 = (payload_len > 3) ? payload[payload_len - 4] : 0;
                uint8_t t1 = (payload_len > 2) ? payload[payload_len - 3] : 0;
                uint8_t t2 = (payload_len > 1) ? payload[payload_len - 2] : 0;
                uint8_t t3 = (payload_len > 0) ? payload[payload_len - 1] : 0;
                ESP_LOGW(TAG, "bad packet: error bit set (hdr_len=%u info=0x%02x payload_len=%u pts=%"PRIu32" scr=%"PRIu32")", header_len, header_info, payload_len, strmh->pts, strmh->last_scr);
                ESP_LOGW(TAG, "payload head: %02x %02x %02x %02x %02x %02x %02x %02x  tail: %02x %02x %02x %02x",
                         b0, b1, b2, b3, b4, b5, b6, b7, t0, t1, t2, t3);
